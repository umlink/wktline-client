...
const UserSelect = (props: IPropsSelect) => {
  ...
  const { data, open, setOpen, loading, adLoading, updateTaskActor, onUserChange, loadMore, onKeywordsChange } =
    useService();
  ...
  const items: MenuProps['items'] = data.userList.map((item) => {
    return {
      ...
      label: (
        <Checkbox
          key={item.userId}
          disabled={[taskDetail.task?.creatorId, taskDetail.task?.handlerId].includes(item.userId)}
          className="w-full"
          checked={data.selectedUserIds.includes(item.userId)}
          onChange={(e) => onUserChange(e, item.userId)}
        >
          <Space>
            <Avatar src={item.avatar} />
            <span>
              {item.username}
              {item.id === taskDetail.task?.creatorId ? '（创建者）' : ''}
            </span>
          </Space>
        </Checkbox>
      ),
      ...
      <Dropdown
        ...
        dropdownRender={(menu) => (
          <div style={contentStyle} className={'w-[240px]'}>
            ...
            <div className={'border-t border-t-gray-100 p-2'}>
              <Button type={'primary'} block onClick={updateTaskActor} loading={adLoading}>
                确认
              </Button>
            </div>
          </div>
