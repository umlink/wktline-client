...
const TaskGroupPage = () => {
  ...
  const [editData, setEditData] = useState<{
    index: number;
    data: API.ProjectGroupItem;
  }>();
  const { loading, params, setParams, resetPageNo, groupList, reloadGroupList, updateGroupRow, delGroup } =
    useService();

  const onDelGroup = (id: string, index: number) => {
    modal.confirm({
      title: '警告',
      content: '删除后无法恢复（该分组下含有任务时无法删除）',
      onOk: () => delGroup(id, index),
    });
  };

  const columns: ColumnsType<API.ProjectGroupItem> = [
    {
      title: '排序',
      dataIndex: 'user',
      key: 'user',
    ...
    <div className={'h-full'}>
      <div className={'mb-4 flex items-center justify-between'}>
        <Input
          ...
          placeholder={'根据任务分组名称搜索'}
        />
        <GroupForm callback={resetPageNo}>
          <Button type={'primary'}>添加</Button>
        </GroupForm>
      </div>
      <div className={'h-[calc(100vh-204px)] overflow-y-auto'} ref={ref} onScroll={useScrollMove(ref, reloadGroupList)}>
        <Spin spinning={loading}>
          <Table<API.ProjectGroupItem>
            showHeader={true}
            bordered
            size={'small'}
